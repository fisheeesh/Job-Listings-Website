<template>
    <div class="text-center text-gray-500 py-7 mt-7" v-if="state.isLoading">
        <PulseLoader />
    </div>
            ...
            <div class="px-6 py-8 m-4 mb-4 bg-white border rounded-md shadow-md md:m-0">
                <form @submit.prevent="handleSubmit">
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="form.type" id="type" name="type" class="w-full px-3 py-2 border rounded"
                            required>
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="form.title" type="text" id="name" name="name"
                            class="w-full px-3 py-2 mb-2 border rounded" placeholder="eg. Beautiful Apartment In Miami"
                            required />
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="form.salary" id="salary" name="salary" class="w-full px-3 py-2 border rounded"
                            required>
                    ...
                    <div class="mb-4">
                        <label for="company_description" class="block mb-2 font-bold text-gray-700">Company
                            Description</label>
                    ...
                    <div class="mb-4">
                        <label for="contact_email" class="block mb-2 font-bold text-gray-700">Contact Email</label>
                        <input v-model="form.company.contactEmail" type="email" id="contact_email" name="contact_email"
                            class="w-full px-3 py-2 border rounded" placeholder="Email address for applicants"
                            required />
                    </div>
                    <div class="mb-4">
                        <label for="contact_phone" class="block mb-2 font-bold text-gray-700">Contact Phone</label>
...
onMounted(async () => {
    try {
        ...
        let res = await axios.get(`/api/jobs/${route.params.id}`)
        state.job = res.data
        //populate inputs
...
const handleSubmit = async () => {
    ...
    try {
        let res = await axios.put(`/api/jobs/${route.params.id}`, updatedJob)
        /**
         * * @TODO - show toast
         */
        toast.success('Job Updated Successfully!')
        router.push({ name: 'job-details', params: { id: res.data.id } })
    }
    catch (err) {
